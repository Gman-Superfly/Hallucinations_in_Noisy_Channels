...
| **B. Intermediate jiggling** ($T=T^*$) | Gently jiggle the key while pushing | The motion helps the key slide past the sticking point. |
| **C. Violent shaking** ($T \gg T^*$) | Shake the key wildly | You drop the key or break the lock. |
...
2. **Resonance state ($T=T^*$):** Noise fluctuations can sum with the weak signal. Sometimes the sum crosses the threshold. Once the supported token is selected, it can bias later context toward the supported path.

3. **Swamped state ($T \gg T^*$):** Noise dominates both signal and barrier. The output is dominated by sampling and form-prior structure.
...
When recoverable signal exists, there can be an optimal noise level $\sigma^*$ (equivalently, temperature $T^*$) that maximizes the trade-off between exploration benefit and thermalization cost:
...
*Note on objective:* This decomposition makes explicit why intermediate noise can be useful: it enables error recovery (correction) while limiting drift to the form prior (hallucination). Equivalently, $T^*$ maximizes overall accuracy in the modeled setting: $T^* = \arg\max_T P(\text{correct output} \mid T)$.
...
#### The intermediate regime ($T \approx T^*$)
...
The optimal $T^*$ depends on knowledge state:
...
Intermediate noise ($T=T^*$) can provide the exploration needed to escape these traps. It can enable:
...
Too much noise ($T \gg T^*$) destroys the signal, increasing relaxation toward the form prior.
