...
This document details the **Optimal Noise Principle** derived from an information-theoretic guided framework and supported by recent and historical empirical evidence (Wu & Yao, 2025; Gammaitoni et al., 1998). It explains why deterministic generation ($T=0$) is brittle and why systems require stochasticity to correct errors.
...
**Key point:** Greedy decoding ($T=0$) gets stuck in local minima (the first wrong token). Optimal noise allows the model to escape errors and find the correct path. Too much noise destroys the signal.
...
**Problem:** Greedy decoding ($T=0$) follows the steepest descent. It falls into the first Form Prior Basin it encounters, even when a deeper Knowledge Well exists nearby. Once in the basin, it cannot escape. It is **kinetically trapped**.

**Solution:** Noise ($T>0$) provides **activation energy** to escape shallow basins and find deeper wells.
...
**Key point:** Without noise, the weak signal *never* crosses the threshold. The system defaults to the global attractor (form prior). With optimal noise, the fluctuations sum constructively with the signal to cross the threshold intermittently. With too much noise, the signal is swamped.
...
**The high-temperature phase explores candidate solutions. Cooling then locks in the selected solution.**
...
**Explanation:** The random candidate acts as a **thermal shock**. It kicks the system out of its current basin of attraction (the "fluent hallucination"), forcing it to re-traverse the energy landscape from a different starting point. The verification process exploits the **asymmetry between generation and verification**: discrimination is easier than generation and requires less channel capacity. This supports the claim that noise can induce re-evaluation and error correction.
...
**Explanation:**
-   Truthful outputs are strong attractors (deep valleys). Hallucinations are shallow attractors (scattered).
...
| Verification-First works | Thermal shock forces re-traversal of the energy landscape |
...
**Hallucination often involves a failure of sampling dynamics.**
