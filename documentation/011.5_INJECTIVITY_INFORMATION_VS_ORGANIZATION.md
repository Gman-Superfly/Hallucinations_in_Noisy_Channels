# Information preservation and useful organization differ

**Core thesis**: Preserving information does not imply useful organization. This distinction explains why training matters for representation quality.
...
### 2.1 Information preservation (injectivity): formal result
...
Nikolaou et al. (2025) prove this property for language models under the assumptions in their paper.

Nikolaou et al. (2025) ([arXiv:2510.15511](https://arxiv.org/abs/2510.15511)) establish:
- **Mathematical proof**: Transformer LMs are injective under the cited assumptions.
- **Empirical check**: billions of collision tests reported zero collisions.
- **Training result**: the property is established at initialization and preserved during training in the cited setting.

**What injectivity guarantees:**
- Distinctness: different sequences map to different points.
- No collisions: No information loss from merging
- Invertibility: Input recoverable (in principle, with caveats)

**What injectivity leaves unresolved:**
- Semantic proximity: Similar sequences may be far apart
- Structured organization: Related sequences may be scattered
- Efficient extraction: Features may require nonlinear or high-sample extraction

Implication: if information is preserved but poorly organized, then representation quality depends on whether the preserved information is accessible for the task.
...
1. **Local continuity**: similar inputs map to similar embeddings.
...
- **Poorly organized for semantics**: No semantic structure
...
Injectivity is necessary for useful representations, and it does not supply useful organization by itself.
...
| Belief-state structure | No | Yes, under the cited assumptions |
| Compression | No | Yes |
| Generalization | Poor | Good |

Conclusion: training can transform random storage into organized retrieval.

### 4.2 Universal manifold: trained models share geometry (Jha et al., 2025)

The vec2vec paper ([arXiv:2505.12540](https://arxiv.org/abs/2505.12540)) reports that trained models can share enough geometric structure for unsupervised embedding translation:
...
- The reported models preserve enough overlapping structure for a useful cross-model map.

**Implication**: Organization reflects the structure of what is represented, and training supplies the pressure that learns it.

### 4.3 Platonic representation hypothesis (Huh et al., 2024)

Different architectures, trained on different data, can converge toward related geometric shapes for representing concepts ([arXiv:2405.07987](https://arxiv.org/abs/2405.07987)).

Implication: organization emerges from training and is constrained by what is being represented, while still depending on architecture, modality, data, and task.
...
| Claim | Status | Source |
|-------|--------|--------|
| LMs preserve information under the cited assumptions | Formal result | Nikolaou et al. (2025): mathematical proof + billions of tests |
| Injectivity holds at initialization and after training | Formal result in cited setting | Nikolaou et al. (2025) |
| Training creates organization | Formal and empirical support in cited setting | NextLat: latents converge to belief states |
| Shared organization is measurable | Bounded support | vec2vec: >0.92 similarity across architectures in the reported setting |

**Conclusion**: 
- **Library A (information preserved)**: supported for LMs under the cited assumptions
- **Library B (information organized)**: shown to require training in the cited work

The useful distinction is now clearer: injectivity can preserve information, while organization requires training pressure or architectural constraints.
...
Proper organization (Library B) makes information more retrievable, reducing matching failures.
...
|---------|--------|------------|
| **Information preservation** | Formal result in the cited setting (Nikolaou et al., 2025) | No collisions under the paper's assumptions |
| **Useful Organization** | Requires training (NextLat et al.) | Efficient retrieval |

The representation-quality question is now operational: is the information organized usefully for the task?
...
**Preservation and organization differ.**

- **Formal result in the cited setting**: LMs are injective (Nikolaou et al., 2025).
- **Supported in the cited settings**: training creates organization (NextLat, vec2vec).
